...
class tester:
    ...
    def test_operation_success(self, result : mqtt_operation_queue.QueueResult, test_name: str):
        if (result != mqtt_operation_queue.QueueResult.SUCCESS):
            self.on_application_failure(test_name, ValueError(f"operation was not successful. Result: {result}", test_name))
    ...
    def test_connect_sub_pub_unsub(self):
        ...
        try:
            ...
            self.test_operation_success(mqtt_queue.subscribe(TEST_TOPIC, mqtt.QoS.AT_LEAST_ONCE, None), "test_connect_sub_pub_unsub")
            self.test_operation_success(mqtt_queue.publish(TEST_TOPIC, "hello_world", mqtt.QoS.AT_LEAST_ONCE), "test_connect_sub_pub_unsub")
            ...
            if (return_operation == None or return_operation.type != mqtt_operation_queue.QueueOperationType.SUBSCRIBE):
                self.on_application_failure("test_connect_sub_pub_unsub", ValueError(f"First operation is not subscribe. Type is {return_operation.type}"))
            self.on_queue_sent_future = Future() # reset future
            return_operation = self.on_queue_sent_future.result()
            if (return_operation == None or return_operation.type != mqtt_operation_queue.QueueOperationType.PUBLISH):
                self.on_application_failure("test_connect_sub_pub_unsub", ValueError(f"Second operation is not publish. Type is {return_operation.type}"))
            self.on_queue_sent_future = Future() # reset future
            return_operation = self.on_queue_sent_future.result()
            if (return_operation == None or return_operation.type != mqtt_operation_queue.QueueOperationType.UNSUBSCRIBE):
                self.on_application_failure("test_connect_sub_pub_unsub", ValueError(f"Third operation is not unsubscribe. Type is {return_operation.type}"))
    ...
    def test_drop_back(self):
        ...
        try:
            ...
            queue_builder.with_on_operation_sent_callback(on_queue_sent_future)
            queue_builder.with_on_operation_dropped_callback(on_queue_dropped_future)
            queue_builder.with_queue_limit_size(2).with_queue_limit_behavior(mqtt_operation_queue.LimitBehavior.DROP_BACK)
            mqtt_queue : mqtt_operation_queue.MqttOperationQueue = queue_builder.build()

            # Add the operations to the queue
            self.test_operation_success(mqtt_queue.subscribe(TEST_TOPIC, mqtt.QoS.AT_LEAST_ONCE, None), "test_drop_back")

            # Add 10 publishes
            for i in range(0, 10):
                self.on_queue_dropped_future = Future() # reset future
                self.test_operation_success(mqtt_queue.publish(TEST_TOPIC, "hello_world", mqtt.QoS.AT_LEAST_ONCE), "test_drop_back")
            ...
            if (dropped_operation == None or dropped_operation.type != mqtt_operation_queue.QueueOperationType.PUBLISH):
                self.on_application_failure("test_drop_back", ValueError(f"Dropped operation is not publish. Type is {dropped_operation.type}"))
            ...
            return_operation : mqtt_operation_queue.QueueOperation = self.on_queue_sent_future.result()
            if (return_operation == None or return_operation.type != mqtt_operation_queue.QueueOperationType.SUBSCRIBE):
                self.on_application_failure("test_drop_back", ValueError(f"First operation is not subscribe. Type is {return_operation.type}"))
            self.on_queue_sent_future = Future() # reset future
            return_operation = self.on_queue_sent_future.result()
            if (return_operation == None or return_operation.type != mqtt_operation_queue.QueueOperationType.UNSUBSCRIBE):
                self.on_application_failure("test_drop_back", ValueError(f"Second operation is not unsubscribe. Type is {return_operation.type}"))
    ...
    def test_drop_front(self):
        ...
        try:
            ...
            queue_builder.with_connection(self.connection).with_enable_logging(PRINT_QUEUE_LOGS)
            queue_builder.with_on_operation_dropped_callback(on_queue_dropped_future)
            queue_builder.with_queue_limit_size(2).with_queue_limit_behavior(mqtt_operation_queue.LimitBehavior.DROP_FRONT)
            mqtt_queue : mqtt_operation_queue.MqttOperationQueue = queue_builder.build()

            # Add the operations to the queue
            self.test_operation_success(mqtt_queue.subscribe(TEST_TOPIC, mqtt.QoS.AT_LEAST_ONCE, None), "test_drop_front")
            ...
            self.test_operation_success(mqtt_queue.publish(TEST_TOPIC, "hello_world", mqtt.QoS.AT_LEAST_ONCE), "test_drop_front")
            dropped_operation : mqtt_operation_queue.QueueOperation = self.on_queue_dropped_future.result()
            if (dropped_operation == None or dropped_operation.type != mqtt_operation_queue.QueueOperationType.SUBSCRIBE):
                self.on_application_failure("test_drop_front", ValueError(f"First Dropped operation is not subscribe. Type is {dropped_operation.type}"))
            # second drop
            self.on_queue_dropped_future = Future() # reset future
            self.test_operation_success(mqtt_queue.publish(TEST_TOPIC, "hello_world", mqtt.QoS.AT_LEAST_ONCE), "test_drop_front")
            dropped_operation = self.on_queue_dropped_future.result()
            if (dropped_operation == None or dropped_operation.type != mqtt_operation_queue.QueueOperationType.UNSUBSCRIBE):
                self.on_application_failure("test_drop_front", ValueError(f"First Dropped operation is not unsubscribe. Type is {dropped_operation.type}"))
    ...
    def test_add_front(self):
        ...
        try:
            ...
            # Fill with publishes
            self.test_operation_success(mqtt_queue.publish(TEST_TOPIC, "hello_world", mqtt.QoS.AT_LEAST_ONCE), "test_add_front")
            self.test_operation_success(mqtt_queue.publish(TEST_TOPIC, "hello_world", mqtt.QoS.AT_LEAST_ONCE), "test_add_front")
            ...
            self.test_operation_success(mqtt_queue.subscribe(TEST_TOPIC, mqtt.QoS.AT_LEAST_ONCE, None), "test_add_front")
            ...
            return_operation : mqtt_operation_queue.QueueOperation = self.on_queue_sent_future.result()
            if (return_operation == None or return_operation.type != mqtt_operation_queue.QueueOperationType.SUBSCRIBE):
                self.on_application_failure("test_add_front", ValueError(f"First operation is not subscribe. Type is {return_operation.type}"))
            self.on_queue_sent_future = Future() # reset future
            return_operation = self.on_queue_sent_future.result()
            if (return_operation == None or return_operation.type != mqtt_operation_queue.QueueOperationType.UNSUBSCRIBE):
                self.on_application_failure("test_add_front", ValueError(f"Second operation is not unsubscribe. Type is {return_operation.type}"))
    ...
    def test_add_error(self):
        ...
        try:
            queue_builder = mqtt_operation_queue.MqttOperationQueueBuilder()
            queue_builder.with_connection(self.connection).with_enable_logging(PRINT_QUEUE_LOGS)
            queue_builder.with_queue_limit_size(2).with_queue_limit_behavior(mqtt_operation_queue.LimitBehavior.RETURN_ERROR)
            mqtt_queue : mqtt_operation_queue.MqttOperationQueue = queue_builder.build()
