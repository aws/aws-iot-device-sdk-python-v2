...
First, create an IAM role that will be needed by a fleet provisioning template. Replace RoleName with a name of the role you want to create. 
<pre>
aws iam create-role \
    --role-name [RoleName] \
...
Finally, create the template resource which will be used for provisioning the demo application. This needs to be done only 
...
These instructions assume you have used the following for the template body:
<pre>
...
Create a temporary provisioning claim certificate set:
<pre>
...
To use the sample with this workflow, you'll need to create a certificate signing request that can be used.
...
As with the previous run, we'll create a temporary certificate set from a provisioning claim.  This step can
...
Finally, supply the certificate signing request while invoking the provisioning sample.  As with the previous sample, if
